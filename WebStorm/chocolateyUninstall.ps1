...
$installVersionDir  = Join-Path $installDir  "WebStorm 2016.2"

if (Test-Path ($installVersionDir)) {

	$uninstallExe = (gci "${installDir}/WebStorm 2016.2/bin/Uninstall.exe").FullName | sort -Descending | Select -first 1
